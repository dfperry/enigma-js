var Enigma = function(config) {
    ...
    self.availableRotors = [];
    self.availableReflectors = [];
    self.availableStators = [];

    self.selectedReflector = null;
    self.selectedRotors = [];
    self.selectedStator = null;
    ...
    self.testConfig = function() {
        self.selectedReflector = new Reflector('UKW', 'German Railway', 'CFAEDB', 'A');
        self.selectedStator = new Stator('ETW', 'German Railway', 'ABCDEF');
        self.selectedRotors = [
            new Rotor('I', 'German Railway', 'BECAFD', 'B'),
            new Rotor('II', 'German Railway', 'CADBFE', 'E'),
            new Rotor('III', 'German Railway', 'CDAFBE', 'A')
        ];

        self.selectedReflector.reset(0);
        for( var i = 0, len = self.selectedRotors.length; i < len; i++) {
            self.selectedRotors[i].reset(0);
        }

        //self.plugboard.addConnection('D', 'A');
    };
